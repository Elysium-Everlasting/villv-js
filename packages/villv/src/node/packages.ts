...
import { createFilter, isInNodeModules, safeRealpathSync } from './utils.js'
...
export function findNearestPackageData(
  ...
  const originalBaseDirectory = base

  while (base) {
    if (packageCache) {
      const cached = getFnpdCache(packageCache, base, originalBaseDirectory)
      if (cached) return cached
    }
    ...
    try {
      if (fs.statSync(pkgPath, { throwIfNoEntry: false })?.isFile()) {
        ...
        if (packageCache) {
          setFnpdCache(packageCache, pkgData, base, originalBaseDirectory)
        }
    ...
    const nextBasedir = path.dirname(base)
...
/**
 * Package cache key for {@link findNearestPackageData}
 */
